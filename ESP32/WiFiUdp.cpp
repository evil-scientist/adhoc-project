...
struct sockaddr_in recipient, sender;
...
int WiFiUDP::parsePacket(){
  ...
  if ((len = recvfrom(udp_server, buf, 1460, MSG_DONTWAIT, (struct sockaddr *) &si_other, (socklen_t *)&slen)) == -1){
    ...
    if(errno == EWOULDBLOCK){
      ...
      //Serial.print("could not receive data:");
...
int WiFiUDP::setupUDP(IPAddress ip, uint16_t port){
	
	 IPAddress   bindIP (0,0,0,0);
  ...
  if (sock != -1) {
    ...
    addr.sin_addr.s_addr = (in_addr_t)bindIP;
    if(bind(sock , (struct sockaddr*)&addr, sizeof(addr)) == -1){
       return -1;   
    }
    fcntl(sock, F_SETFL, O_NONBLOCK);
  recipient.sin_addr.s_addr = (uint32_t)ip;
  recipient.sin_family = AF_INET;
...
int WiFiUDP::readUDP(char buf[]){
	return recvfrom(sock, buf, 1460, MSG_DONTWAIT, (struct sockaddr *) &sender, (socklen_t *)&slen);
}

int WiFiUDP::writeUDP(byte buf[], int length){
	return sendto(sock, buf, length, 0, (struct sockaddr*) &recipient, sizeof(recipient));
}